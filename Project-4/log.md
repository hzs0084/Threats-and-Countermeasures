...
```bash
sudo mkdir -p /var/www/html/{admin,uploads,backups,assets/css,assets/js}
...
EOF
```
## Step 4 — Admin login (SQLi vulnerable)
```bash
sudo tee /var/www/html/admin/login.php > /dev/null << 'EOF'
<?php
session_start();
include '../config.php';
...
```bash
sudo tee /var/www/html/admin/dashboard.php > /dev/null << 'EOF'
<?php
session_start();
include '../config.php';
...
## Logout and uploads placeholder
```bash
sudo tee /var/www/html/admin/logout.php > /dev/null << 'EOF'
<?php
session_start();
session_destroy();
header('Location: login.php');
...
# Placeholder so /uploads/ isn't a 403
sudo tee /var/www/html/uploads/index.php > /dev/null << 'EOF'
<?php http_response_code(403); die('Forbidden'); ?>
EOF

# Placeholder so /backups/ isn't a 403
sudo tee /var/www/html/backups/index.php > /dev/null << 'EOF'
<?php http_response_code(403); die('Forbidden'); ?>
EOF
```
...
# ----------------------------------------------------------------
# DB config - hardcoded for internal deployment
# ----------------------------------------------------------------
DB_HOST = 'localhost'
DB_USER = 'acme_app'
DB_PASS = 'Acm3C0rp2024!'
DB_NAME = 'acme_db'

# API key - validated on some routes but NOT enforced on /api/user
# TODO: enforce on all routes before public launch - fong
VALID_API_KEY = 'sk-acme-dGhpcyBpcyBhIHRlc3Qga2V5'
...
def health():
    return jsonify({
        'status': 'ok',
        'service': 'ACME Corp Internal API',
        'version': '1.2.4'
    })

# ----------------------------------------------------------------
# IDOR vulnerability - no authentication check on user lookup
# Any caller can enumerate /api/user/1 through /api/user/N
# The API key header is read but never validated here - intentional
# ----------------------------------------------------------------
...
# ----------------------------------------------------------------
# Employee list - requires API key (but key is in the repo)
# ----------------------------------------------------------------
...
def list_employees():
    ...
    try:
        db  = get_db()
        cur = db.cursor()
        cur.execute("SELECT id, name, role, department FROM employees")
        rows = cur.fetchall()
        cur.close()
        db.close()
...
# ----------------------------------------------------------------
# Orders endpoint - requires API key
# ----------------------------------------------------------------
...
def list_orders():
    ...
    try:
        db  = get_db()
        cur = db.cursor()
        cur.execute("SELECT id, customer_name, product, amount, status FROM orders")
        rows = cur.fetchall()
        cur.close()
        db.close()
...
# ----------------------------------------------------------------
# Red herring endpoint - exists in comments in config.php
# returns nothing useful, just frustrates LLM-assisted students
# ----------------------------------------------------------------
@app.route('/api/admin/export', methods=['POST'])
def admin_export():
    # Legacy endpoint - disabled pending security review
    # Requires internal cert auth - not implemented yet
    return jsonify({'error': 'Endpoint disabled. Contact mallory@acmecorp.internal'}), 403
